...
class ProcessLayoutTest(TestCase):
    def test_three_service_directories_have_independent_entrypoints(self) -> None:
        for service in ("asr", "intent", "video"):
    ...
    def test_supervisor_starts_exactly_three_service_programs(self) -> None:
        ...
        self.assertEqual({"program:asr", "program:intent", "program:video"}, programs)
        ...
        self.assertIn("services.video.main", parser["program:video"]["command"])
